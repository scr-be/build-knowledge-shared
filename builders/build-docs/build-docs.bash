...
REPOS=(
    ...
    "wonka-library",
    "wonka-bundle",
    "arthur-doctrine-library",
    "arthur-doctrine-uuid-library",
    "arthur-doctrine-fixtures-library",
    "teavee-html-generator-bundle",
    "teavee-google-web-fonts-bundle",
    "cache-bundle",
    "swim-bundle",
    "mantle-bundle", 
    "batch-uploader-bundle"
)
...
function clone_one()
{
    ...
    then
        ...
        if [ -z ${cfg_pkg_api_parser+x} ]
        then
            echo "         ! [  stop  ] No parser configured."
            return

        elif [ "${cfg_pkg_api_parser}" == "sami" ] || [ "${cfg_pkg_api_parser}" == "~" ]
        then
        ...
        if [ -z ${cfg_pkg_api_branch+x} ] || [ "${cfg_pkg_api_branch}" == "~" ]
        then
            echo "         - [  conf  ] Git Branch      : 'origin/master'."
            cfg_pkg_api_branch="origin/master"
        fi

        if [ -z ${cfg_pkg_api_cfgmod} ] || [ "${cfg_pkg_api_cfgmod}" == "clone" ] || [ "${cfg_pkg_api_cfgmod}" == "~" ]
        then
            echo "         - [  conf  ] Config Mode     : 'git remote clone'."
        elif [ "${cfg_pkg_api_cfgmod}" == "submodule" ]
        then
        ...
        if [ ! -z ${cfg_pkg_api_runner} ] && [ "${cfg_pkg_api_runner}" != "~" ]
        then
    ...
    if [ "${cfg_pkg_api_cfgmod}" == "submodule" ]
    then
        ...
        if [ -z ${cfg_pkg_api_runner} ] || [ "${cfg_pkg_api_runner}" == "~" ] || [ "${cfg_pkg_api_runner}" == "" ]
        then
